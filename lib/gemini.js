...
// request is exactly the note. Returns { post, sources, confirmed }:
...
export async function draftPost(note, news = []) {
  if (!news.length) {
    return { post: await generate(systemInstruction, note), sources: [], confirmed: true };
  }
  return splitSources(await generate(systemInstruction, withNews(note, news)), news);
}
...
- Where a headline is relevant, work its information into the post to strengthen or update the note's point. Skip headlines that aren't relevant.
...
async function generate(instruction, note, generationConfig) {
  ...
  for (const { model, waitMs } of attempts) {
    ...
    res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`, request);
